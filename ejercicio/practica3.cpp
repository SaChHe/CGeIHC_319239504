...
prisma rectangula/cubo
cono
cilindros
esfera
toroide

para la esfera hay dos resoluciones: stacks (paralelos) y slices (meridianos)
esto se hace con dos for anidados
...
Sphere sp = Sphere(1.0, 100, 100); //recibe radio, slices, stacks a mas vertices mas esferico
...
void CrearCilindro(int res, float R) {
	...
	for (n = 0; n <= (res); n++) {
		...
		for (i = 0; i < 3; i++) {
			switch (i) {
			case 0:
				vertices.push_back(x);				//circunferencia inferior
				break;
			case 1:
...
void CrearCono(int res, float R) {
	...
	vertices.push_back(-0.5);//almacena en una lista de geometría todos los puntos de la circunferencia
	...
	Mesh* cono = new Mesh();
	cono->CreateMeshGeometry(vertices, indices, vertices.size(), res + 2); //nuevo mesh para dibujar
...
int main()
{
	...
	*/
	...
	while (!mainWindow.getShouldClose())
	{
		...
		//con cilindro y cono se ocupa Geometry o se veran muy feos 
		...
		//piso
		model = glm::mat4(1.0f);
		color = glm::vec3(0.0f, 0.128f, 0.0f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		...
		color = glm::vec3(0.0f, 1.0f, 0.0f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		model = glm::translate(model, glm::vec3(0.25f, 0.15f, -2.55f));
		model = glm::scale(model, glm::vec3(0.2f, 0.2f, 0.2f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		meshList[0]->RenderMeshGeometry();
		...
		model = glm::mat4(1.0f);
		color = glm::vec3(0.128f, 0.128f, 0.0f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		...
		color = glm::vec3(0.0f, 1.0f, 0.0f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		model = glm::translate(model, glm::vec3(0.8f, -0.2f, -3.0f));
		model = glm::scale(model, glm::vec3(0.1f, 0.5f, 0.2f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		...
		//ventanas lterales izquierda
		model = glm::mat4(1.0f);
		color = glm::vec3(0.0f, 1.0f, 0.0f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		model = glm::translate(model, glm::vec3(-0.45f, 0.14f, -2.7f));
		model = glm::scale(model, glm::vec3(0.2f, 0.2f, 0.2f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		meshList[0]->RenderMeshGeometry();
		...
		model = glm::mat4(1.0f);
		color = glm::vec3(0.0f, 1.0f, 0.0f);
		model = glm::translate(model, glm::vec3(-0.45f, 0.12f, -3.3f));
		model = glm::scale(model, glm::vec3(0.2f, 0.2f, 0.2f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		meshList[0]->RenderMeshGeometry();

		//ventanas lterales derechas
		model = glm::mat4(1.0f);
		color = glm::vec3(0.0f, 1.0f, 0.0f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		model = glm::translate(model, glm::vec3(0.45f, 0.12f, -2.7f));
		model = glm::scale(model, glm::vec3(0.2f, 0.2f, 0.2f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		meshList[0]->RenderMeshGeometry();
		...
		color = glm::vec3(0.0f, 1.0f, 0.0f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		model = glm::translate(model, glm::vec3(0.45f, 0.15f, -3.3f));
		model = glm::scale(model, glm::vec3(0.2f, 0.2f, 0.2f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		meshList[0]->RenderMeshGeometry();

		//esfera posterior azul
		model = glm::mat4(1.0f);
		color = glm::vec3(0.0f, 0.0f, 1.0f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		model = glm::translate(model, glm::vec3(0.0f, 0.0f, -3.35f));
